/**
 * Session memory prompt templates for Xethryon.
 * Ported from cc-leak/src/services/SessionMemory/prompts.ts.
 *
 * Adapted: Replaced getClaudeConfigHomeDir with ~/.xethryon,
 * replaced roughTokenCountEstimation with Math.ceil(len/4).
 */
...
function getXethryonConfigDir(): string {
...
export async function buildSessionMemoryUpdatePrompt(
  ...
  return substituteVariables(promptTemplate, variables) + sectionReminders
}
...
function flushSessionSection(
  ...
  if (!sectionHeader) return { lines: sectionLines, wasTruncated: false }
